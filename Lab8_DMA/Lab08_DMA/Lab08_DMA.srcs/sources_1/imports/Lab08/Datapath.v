...
module Datapath(
  input reset_n, clk, ICacheBusy, DCacheBusy,                                                 // used for reset PC and RF write
  ...
  output PCUpdate, isFlush, isInvalid, is_halted, IFFlush,
  output [3:0] opcode,
  output [5:0] func,
...
// Stall conditions
assign IStall = ICacheBusy;
assign DStall = DCacheBusy;
assign PCUpdate = (!IStall || (IStall && IFFlush)) && !DStall && !RAWStall;
...
PCResolveUnit PCR(.offset(Offset_ID), .PredPC(PredPC_ID), .OldPC(PC_ID), .taken(taken), .ResPC(ResPC), .IFFlush(IFFlush), .Btype(opcode[1:0]), .ReadA(ReadA_fwd_ID), .ReadB(ReadB_fwd_ID), .PCSrc(PCSrc), .isInvalid(isInvalid), .clk(clk));
