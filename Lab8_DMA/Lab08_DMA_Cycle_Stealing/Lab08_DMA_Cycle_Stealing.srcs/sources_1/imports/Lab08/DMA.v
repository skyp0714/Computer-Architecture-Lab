...
module DMA (
    ...
    always @(posedge CLK) begin
        ...
        if(interrupt) begin                                 // deassert interrupt signal after one cycle
            interrupt <= 0;
            offset <= 2'dz;
        end else if(cmd && BG && !BR && offset == 0) begin  // generate end signal
            interrupt <= 1;
            WRITE <= 1'bz;
        end else if(cmd && !BG) begin                       // DMA is started or re-started after the cycle stealing
            BR <= 1;
            delay <= 0;
            if(offset)begin
                WRITE <= 1;
            ...
            BR <= 0;
            offset <= 0;
        end else if(BG && BR) begin                         // sending 4words
            WRITE <= 1;
            delay <= delay + 1;
            if(delay == 3) begin                            // send next offset
                offset <= offset + 1;
                BR <= 0;
                WRITE <= 1'bz;
            end
