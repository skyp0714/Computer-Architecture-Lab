...
module Datapath(
  ...
  output PCUpdate, isFlush, isStall, is_halted,
  output [3:0] opcode,
  output [5:0] func,
...
// IF stage
BTB BTB(.clk(clk), .reset_n(reset_n), .taken(taken), .addr(PC_ID), .PC(PC), .ResPC(ResPC), .PredPC(PredPC), .isFlush(isFlush_ID), .isStall(stall));
...
PCResolveUnit PCR(.offset(Offset_ID), .PredPC(PredPC_ID), .OldPC(PC_ID), .taken(taken), .ResPC(ResPC), .IFFlush(IFFlush), .Btype(opcode[1:0]), .ReadA(ReadA_fwd_ID), .ReadB(ReadB_fwd_ID), .PCSrc(PCSrc), .isFlush(isFlush_ID), .isStall(stall));
HazardDetectionUnit HDU(.dest({Dest_EX, Dest_MEM, Dest_WB}), .RegWrite({RegWrite_EX, RegWrite_MEM, RegWrite_WB}), .Inst(Inst_ID), .stall(stall), .isFlush(isFlush_ID), .isLWD_EX(isLWD_EX), .isLWD_MEM(isLWD_MEM));
